...
/**
 * Seconds per animation loop for the wind-flow visualization, so faster wind
 * visibly reads as faster wind. Clamped so 12kt isn't static and 35kt isn't
 * an unreadable blur.
 */
export function getFlowDurationSeconds(speedKnots: number): number {
  const duration = 3.1 - speedKnots * 0.07;
  return Math.min(2.6, Math.max(0.85, duration));
}
 ...
 * rating. Used to pick the featured spot on the list page, not shown to users
 * as a number.
 */
